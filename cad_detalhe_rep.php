...
$produtos = "SELECT idusuario, nome, email, login, cpf, street, street_number, neighborhood, city, state, documento_url, cep, pagamento_id, pagamento_secret FROM usuario WHERE idusuario = $index;";
...
$i = 0 ;
  ...
  $pagamentosecret = $row['pagamento_secret'];
  $cep = $row['cep'];
  $street = $row['street'];
...
if($state === 'TO') {
  ...
  echo "<input id='pagamentoid' name='pagamentoid' class='prodDetailInput centAlign' value='$pagamentoid' />";
  echo "<label for='pagamentosecret'>Secret Gerencianet</label>";
  echo "<input id='pagamentosecret' name='pagamentosecret' class='prodDetailInput centAlign' value='$pagamentosecret' />";
}
}
